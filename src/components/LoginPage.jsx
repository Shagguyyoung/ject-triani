...
export default function LoginPage() {
  ...
  console.log("Auth0 chargé, isLoading:", isLoading);

  return (
    <div className="min-h-screen bg-[#0a1628] flex items-center justify-center"
      style={{ fontFamily: "'Montserrat', sans-serif" }}>
      <div className="bg-[#0d1e38] border border-[#d4af7a33] rounded-xl p-10 w-full max-w-sm text-center">
        <p className="text-3xl text-[#f0e8d6] mb-2"
          style={{ fontFamily: "'Cormorant Garamond', serif" }}>
          JECT-TRIANI
        </p>
        <p className="text-xs text-[#8899aa] tracking-widest uppercase mb-8">
          Espace responsables
        </p>
        <button
          onClick={() => {
            console.log("Clic — lancement loginWithRedirect");
            loginWithRedirect();
          }}
          disabled={isLoading}
